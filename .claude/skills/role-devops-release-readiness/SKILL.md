...
description: Reviews CI/CD pipeline integrity, migration rollout safety, background-job observability across all 18 Hangfire jobs, security-event alerting (distinct from job-failure alerting — failed logins, Admin role changes), and incident runbooks against DORA's five software-delivery-performance metrics and supply-chain/config-security guidance. Covers role 13 (DevOps / SRE / Release Engineer) from the MusicLounge SDLC role charter. Use when asked to review deployment readiness, CI/CD, monitoring, alerting, rollback plans, or explicitly invoke the DevOps/SRE role.
...
Work in order: **(1) Measure DORA metrics → (2) Check deploy safety → (3) Audit background-job observability → (4) Audit security-event alerting → (5) Check runbooks → (6) Check supply chain → (7) Report.**
...
## 4. Audit security-event alerting — distinct from job-failure alerting

A job silently failing and a security-relevant event going unnoticed are different failure classes with different owners and different urgency. Confirm specifically that repeated failed logins, permission-denial spikes, and any change to a user's Role (especially promotion to Admin, which currently has no API path and must go through direct SQL — exactly the kind of action that should page someone if it happens outside a known maintenance window) are wired to actual alerting, not merely written to a Serilog file nobody is watching. Logging an event and alerting on it are not the same control — a log entry only helps after the fact, once someone thinks to look; alerting is what catches it while it's still actionable.

## 5. Check incident runbooks
...
State DORA metrics with explicit confidence/derivation method, list every job without alerting by name, list any security-relevant event without alerting by name, and attach draft runbooks for any missing ones rather than only flagging the gap.
